...
public:
	...
	uint32 GetNum() const { 
#if SL_USE_MOBILEHZB
		return MobileHzbPrimitives.Num();
#else
...
private:
	...
	bool IsInvalidFrame(uint32 FrameNumber = 0) const;
	...
	void SetInvalidFrameNumber(uint32 FrameNumber = 0);
...
public:
	...
	enum { MobileSizeX = 256 };
	enum { MobileSizeY = 256 };
	TArray<FOcclusionPrimitive>	MobileHzbPrimitives;
...
public:
	...
	TUniformBufferRef<FViewUniformShaderParameters> VolumetricCloudViewUniformBuffer;
	//@StarLight code -  Mobile Volumetric Cloud, Added by zhouningwei
...
protected:
	...
	//@StarLight code -  Mobile Volumetric Cloud, Added by zhouningwei
	void InitVolumetricCloudView(FRHICommandListImmediate& RHICmdList, FViewInfo& View);

	void PrecomputeVolumetricCloud(FRHICommandListImmediate& RHICmdList, FScene* InScene, FViewInfo& View);

	void HalfResDepthSurfaceCheckerboardMinMax(FRHICommandListImmediate& RHICmdList, const FViewInfo& View);

	/** Renders volumetric cloud*/
	void RenderVolumetricCloud(FRHICommandListImmediate& RHICmdList, const FViewInfo& View);

	void ReconstructVolumetricCloud(FRHICommandListImmediate& RHICmdList, const FViewInfo& View);

	/** Blend volumetric cloud to scenecolor*/
	void ComposeVolumetricCloudToScene(FRHICommandListImmediate& RHICmdList,const FViewInfo& View);
	//@StarLight code -  Mobile Volumetric Cloud, Added by zhouningwei
