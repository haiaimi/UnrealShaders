...
IMPLEMENT_SHADER_TYPE(, FApplyForcePS, TEXT("/SLShaders/InteractiveWater.usf"), TEXT("ApplyForcePS"), SF_Pixel);
...
IMPLEMENT_SHADER_TYPE(, FUpdateHeightFieldPS, TEXT("/SLShaders/InteractiveWater.usf"), TEXT("UpdateHeightFieldPS"), SF_Pixel);
...
IMPLEMENT_SHADER_TYPE(, FComputeNormalPS, TEXT("/SLShaders/InteractiveWater.usf"), TEXT("ComputeNormalPS"), SF_Pixel);
...
void FInteractiveWater::UpdateForceParams(float InDeltaTime, FVector2D CurDir, FVector CenterPos, float AreaSize, const TArray<FVector>& AllForce)
{
	...
	for (auto& Iter : AllForce)
	{
		...
		{
			ForcePointParams.Add(FVector4(UVToHeightField + CurDir.GetSafeNormal() * 0.005f, FVector2D(0.01f, 1.f)));
		}
	}
}
