...
void UWaveTrigger_SkeletalMesh::BeginPlay()
{
	...
	{ 
		...
		{
			if (InteractiveWaterSubsystem->CurWaterMesh == nullptr && Iter->GetBodyInstance()->GetSimplePhysicalMaterial() == WaterPhysicMaterial)
			{
				InteractiveWaterSubsystem->CurWaterMesh = Cast<UStaticMeshComponent>(Iter);
			}

			if (InteractiveWaterSubsystem->CurWaterMesh == Iter)
			{
